...
namespace Basic_Photo_Editor
{
    public partial class LayerContainer : UserControl
    {
        ...
        public Paint_Tools.Tools Tool { get; set; }

        public void ProcessUpdate(Bitmap processing, bool preview = false, bool filter = false)
        {
            if (processing != null)
            {
                if (!preview && Tool.Tool != Basic_Photo_Editor.Paint_Tools.Tool.Transform) layers[current].Layer.Stacking();
                using (Graphics g = Graphics.FromImage(layers[current].Layer.Image))
                {
                    if (preview || filter || Tool.Tool == Basic_Photo_Editor.Paint_Tools.Tool.Eraser)
